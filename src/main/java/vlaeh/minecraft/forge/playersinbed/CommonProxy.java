...
public class CommonProxy {
    	...
    	if (playersInBed.contains(name)) {
    		...
    		if ((entityPlayer.world.getWorldTime() % 24000L) != 0) {
    			...
    			if (ratio >= PlayerInBed.ratio)
    				sendMessageToPlayers(entityPlayer.world, new TextComponentString(PlayerInBed.i18n.format("playerinbed.leftbedOK", name, ratio)));
    			else
    				sendMessageToPlayers(entityPlayer.world, new TextComponentString(PlayerInBed.i18n.format("playerinbed.leftbedKO", name, ratio)));
    		}
    	}
    }
    ...
    public void onSleepingLocationCheckEvent(final SleepingLocationCheckEvent event) {
    	...
    	if (! playersInBed.contains(name)) {
			...
			if (ratio >= PlayerInBed.ratio)
				sendMessageToPlayers(entityPlayer.world, new TextComponentString(PlayerInBed.i18n.format("playerinbed.isinbedOK", name, ratio)));
			else
				sendMessageToPlayers(entityPlayer.world, new TextComponentString(PlayerInBed.i18n.format("playerinbed.isinbedKO", name, ratio)));
    	}

    	if (PlayerInBed.skipEnabled && (ratio >= PlayerInBed.ratio) && (ratio < 100) && entityPlayer.isPlayerFullyAsleep()) {
